...
// ----------------------------------------------------------------------
// MIGRATION STATUS AND THEORY FREEZE
//   This file extracts the *historical MolSpin algebra* from task-local NZ code
//   into a reusable SpinAPI primitive.  It is intentionally NOT yet a new
//   "improved" NZ theory and it must not silently change published behavior.
//
//   Historical MolSpin/JACS workflow:
...
//
//   This historical form underlies the MolSpin workflow used in
...
//
//   Those are not algebraically interchangeable.  Production migration must
//   first reproduce historical outputs exactly; a reactive-L0 variant may be
...
namespace SpinAPI::NakajimaZwanzig
{
    ...
    bool HistoricalFrequencyMatrix(const arma::vec &eigenvalues,
        arma::cx_mat &omega,std::string *error)
    {
        ...
        // Historical MolSpin lambda = kron(E,I)-kron(I,E^T).
        omega=arma::kron(E,I)-arma::kron(I,E.st());
        return true;
    }

    bool HistoricalExponentialSpectralDensity(const std::complex<double> &amplitude,
        ...
        arma::cx_mat &spectralDensity,std::string *error)
    {
        ...
        if(omega.n_rows==0||omega.n_rows!=omega.n_cols||
            ...
            return Fail(error,"invalid historical NZ exponential spectral-density inputs");
    ...
    bool HistoricalMultiExponentialSpectralDensity(const std::vector<double> &amplitudes,
        const std::vector<double> &tauC,const arma::cx_mat &omega,
        arma::cx_mat &spectralDensity,std::string *error)
    {
        if(error)error->clear();
        if(amplitudes.empty()||amplitudes.size()!=tauC.size())
            return Fail(error,"historical NZ multi-exponential lists must be equally sized and non-empty");
        arma::cx_vec entries(omega.n_rows,arma::fill::zeros);
        for(arma::uword i=0;i<entries.n_elem;++i)
        {
            for(size_t j=0;j<tauC.size();++j)
            {
                if(!std::isfinite(amplitudes[j])||!std::isfinite(tauC[j])||tauC[j]==0.0)
                    return Fail(error,"historical NZ amplitudes/tau_c must be finite with nonzero tau_c");
    ...
    bool HistoricalTensor(const arma::cx_mat &op1,const arma::cx_mat &op2,
        const arma::cx_mat &spectralDensity,arma::cx_mat &tensor,std::string *error)
    {
        ...
        if(op1.n_rows==0||op1.n_rows!=op1.n_cols||arma::size(op1)!=arma::size(op2))
            return Fail(error,"historical NZ operators must be equally sized square matrices");
        const arma::uword d=op1.n_rows;
        if(spectralDensity.n_rows!=d*d||spectralDensity.n_cols!=d*d)
            return Fail(error,"historical NZ spectral density has incompatible superspace dimension");
